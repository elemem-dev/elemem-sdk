...
| 参数 | 描述 |
|------|------|
| `--record <format>` | 录制格式：`0`=bin，`1`=json（当前仅支持bin） |
| `--record_file <path>` | 录制文件路径（必须为绝对路径） |
| `--record_skip` | 跳过payload录制，仅记录头部和时间戳 |
| `--stop_record` | 停止录制 |
...
| 参数 | 描述 |
|------|------|
| `--play <format>` | 回放格式：`0`=bin，`1`=json（当前仅支持bin） |
| `--play_file <path>` | 回放文件路径（支持相对路径） |
| `--play_filler <opcode>` | 指定回放的opcode类型（十六进制或十进制） |
| `--play_mode <mode>` | 回放模式：`0`=normal，`1`=huge |
| `--play_loop <times>` | 回放次数（默认为1） |
...
```bash
# 完整录制
elem-smi --record 0 --record_file /tmp/record.bin

# 仅录制头部信息（跳过payload）
elem-smi --record 0 --record_file /tmp/record.bin --record_skip
```
...
#### 基本回放
```bash
elem-smi --play 0 --play_file /tmp/record.bin
```

#### 高级回放
```bash
# 指定opcode、模式和循环次数
elem-smi --play 0 --play_file /tmp/record.bin --play_filler 0xb0 --play_mode 1 --play_loop 2
```
...
**注意：** 总采样时间（buff_stat × buff_interval）应大于1秒
